...
output_size = 6 # 情感分类的类别数
num_layers = 3
model = Bert_LSTM_Model(input_size, hidden_size, output_size, num_layers, False).to(device)

model.load_state_dict(torch.load("D:/桌面/Emotional_classification/Bert-LSTM/model/6.pth"))
...
print("loss: {}, rec: {}, acc: {}, pre: {}, F1: {}".format(loss / len(train_loader), rec, acc, pre, F1))
