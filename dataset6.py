...
def load_data(data_path, data_stop_path):
    ...
    data_list = open(data_path, encoding='utf-8')  # 构建数据列表
    ...
    for i in range(len(data_json)):
        ...
        if label == 'neutral':
            label = 5
        record.append([content, label])
    max_len_seq = 0
    data = []
    for content, label in record:
        ...
        if max_len_seq < len(seg_res):
            max_len_seq = len(seg_res)  # 最大序列长度
...
def data_loader(dataset, config):
    return DataLoader(dataset, batch_size=config.batch_size, shuffle=False, drop_last=True)